...
本项目是一个混合 AI 代理服务器，结合了 `deepseek-r1` 模型的思考能力和 `gemini-exp-1206` 模型的强大语料库与多模态能力。它可以接收类似 OpenAI API 格式的请求，支持文本对话和图像识别，并返回流式响应。该模型支持自动联网搜索学术信息。

## 特性

- **混合模型架构**：
  - 使用 DeepSeek-R1 作为前置思考系统（支持 7B 到 671B 不同参数规模）
  - 使用 Gemini-1206 作为主要输出模型
  - 支持模型故障自动切换和重试机制
